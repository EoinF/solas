...
TEST_F(BoundRayCast_BrightnessTest, brightness_at_center)
{
    ...
    lightmapManager->addLight((LIGHT_X + 0.5f) * TILE_SIZE, (LIGHT_Y + 0.5f) * TILE_SIZE, PI * 3, RANGE_IN_TILES * TILE_SIZE, glm::vec2(1, 0), BRIGHTNESS);
...
TEST_F(BoundRayCast_BrightnessTest, brightness_smooth_at_span_edge)
{
    ...
    const auto LIGHT_X = 22;
    const auto LIGHT_Y = 10;
    lightmapManager->addLight((LIGHT_X + 0.5f) * TILE_SIZE, (LIGHT_Y + 0.5f) * TILE_SIZE, PI, RANGE_IN_TILES * TILE_SIZE);
    lightmapManager->update();
