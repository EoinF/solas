...
TEST_F(BoundRayCast_BrightnessTest, brightness_decreases) {
	...
	int brightnessAtEdge =
		lightmapManager->getTileState(LIGHT_X + RANGE_IN_TILES, LIGHT_Y).brightness();
	...
	for (auto lightData : orderedLightData) {
		EXPECT_TRUE(previousLightData.brightness <= lightData.brightness
						...
						: ::testing::AssertionFailure()
							  ...
							  << lightData.distance << " | " << lightData.brightness << std::endl
							  << "(" << previousLightData.x << "," << previousLightData.y << ") | "
							  << previousLightData.distance << " | " << previousLightData.brightness
							  << std::endl);
...
TEST_F(BoundRayCast_BrightnessTest, brightness_smooth_at_span_edge) {
	...
	int brightnessOutsideEdge =
		lightmapManager->getTileState(LIGHT_X - 1, LIGHT_Y - 2).brightness();
