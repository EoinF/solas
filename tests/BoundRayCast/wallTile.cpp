...
TEST_F(BoundRayCast_WallTileTest, wall_blocks_light_pointing_left) {
	...
	lightmapManager->addLight((WALL_X + 2) * TILE_SIZE, WALL_Y * TILE_SIZE, 2 * PI,
...
TEST_F(BoundRayCast_WallTileTest, wall_blocks_light_pointing_down) {
	...
	lightmapManager->addLight(WALL_X * TILE_SIZE, (WALL_Y - 2) * TILE_SIZE, 2 * PI,
							  RANGE_IN_TILES * TILE_SIZE);
	lightmapManager->update();
...
TEST_F(BoundRayCast_WallTileTest, wall_blocks_light_pointing_northwest) {
	...
	lightmapManager->addLight((WALL_X + 1) * TILE_SIZE, (WALL_Y + 1) * TILE_SIZE, 2 * PI,
							  RANGE_IN_TILES * TILE_SIZE);
	lightmapManager->update();
