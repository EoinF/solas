...
TEST_F(BoundRayCast_UpdateLightTest, update_light_position) {
	int LIGHT_X = 22;
	int LIGHT_Y = 10;
	...
	lightmapManager->updateLightPosition(lightId, UPDATED_LIGHT_X * TILE_SIZE,
										 UPDATED_LIGHT_Y * TILE_SIZE);
	lightmapManager->update();
...
TEST_F(BoundRayCast_UpdateLightTest, update_light_direction) {
	...
	lightmapManager->update();
	lightmapManager->updateLightDirection(lightId, glm::vec2(-1, 0));
	lightmapManager->update();
...
TEST_F(BoundRayCast_UpdateLightTest, update_light_span) {
	...
	lightmapManager->update();
	lightmapManager->updateLightSpan(lightId, UPDATED_SPAN);
	lightmapManager->update();
